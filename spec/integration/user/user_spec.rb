...
describe User, :aggregate_failures, :db do
  describe '.create' do
    it 'adds a new user ' do
      user = User.create 'Bob', 'test@example.com', 'pswd123'
    ...
    it "hashes user's password" do
      ...
      User.create 'Bob', 'test_bcrypt@example.com', 'pswd123'
    end
  end

  describe '.find' do
    let(:user) { User.create 'Bob', 'test_find@example.com', 'pswd123' }

    example 'a specific user with given ID' do
      user = User.create 'Bob', 'test_find@example.com', 'pswd123'
  ...
  describe '#==' do
    example 'two users are equal if their ids and emails match' do
      user1 = User.new 1, 'Bob', 'test_eql1@example.com'
      user2 = User.new 1, 'Bob', 'test_eql1@example.com'
      expect(user1).to eq user2
    end

    example 'two users are not equal if their values are different' do
      user1 = User.new 1, 'Bob', 'test_eql1@example.com'
      user2 = User.new 1, 'Bob', 'test_eql2@example.com'
